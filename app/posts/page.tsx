...
async function PostsList({ searchParams }: { searchParams: any }) {
  ...
  try {
    ...
    // 검색어 필터링
    if (search) {
      postsQuery = postsQuery.or(`title.ilike.%${search}%,content.ilike.%${search}%,excerpt.ilike.%${search}%`);
    }
    ...
    <div className="space-y-8">
      ...
      <section className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-6 border-t">
          ...
          {/* 검색 박스 */}
          <SearchInput initialSearchQuery={search} />
        </div>
      </section>
  ...
  return (
    <div>
      ...
      <section className="text-center mb-16">
        ...
        <p className="text-lg text-purple-500 max-w-2xl mx-auto font-medium">
          심리학, 뇌과학, 인간관계, 독서 등 자기계발에 관한 다양한 글을 확인해보세요.<br/>
      ...
      <Suspense fallback={
        <div className="text-center py-16">
          ...
          <p className="text-purple-400">포스트를 불러오는 중...</p>
        </div>
      }>
